...
gseaplot2 <- function(x, geneSetID, title = "", color="green", base_size = 11,
  ...
  p.res <- p.res + ylab("Running\nEnrichment Score") +
    theme(axis.text.x=element_blank(),
          axis.ticks.x=element_blank(),
          axis.line.x=element_blank(),
          plot.margin=margin(t=.5, r = .5, b=0, l=.5, unit="mm")) +
    geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.2) ## added by PP
  ...
  p2 <- ggplot(gsdata, aes_(x = ~x)) +
    geom_linerange(aes_(ymin=~ymin, ymax=~ymax, color=~Description),linewidth = 0.05) +
  ...
  p.pos <- p.pos + ylab("Ranked list metric") +
    ...
    theme(plot.margin=margin(t = -.1, r = .2, b=.2, l=.2, unit="mm"),
          axis.title = element_text(size=base_size)) #added by PP
